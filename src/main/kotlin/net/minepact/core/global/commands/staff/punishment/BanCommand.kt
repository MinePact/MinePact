...
class BanCommand : Command(
    ...
    permission = Permission("minepact.punishments.ban"),
    ...
    override fun execute(
        ...
        try {
            ...
            PunishmentRepository.findActiveByTargetAndType(it.data.uuid, PunishmentType.BAN).thenAccept { activeBan ->
                if (activeBan != null && activeBan.expiresAt >= expiresAt && expiresAt != Long.MIN_VALUE) {
                    sender.sendMessage(
                        "<white>$targetName <red>is already banned. Their current ban expires at <white>${formatDate(activeBan.expiresAt)}<red>."
                    )
                    return@thenAccept
                }

                if (activeBan != null && activeBan.expiresAt < expiresAt) {
                    val reverted = revertPunishment(activeBan, sender.data.uuid, "OTHER_BAN_OVERRIDE")
                ...
                PUNISHMENTS_WEBHOOK.sendMessage("", listOf(banEmbed(punishment, listOf(scope, announcement))))
            }
        }
