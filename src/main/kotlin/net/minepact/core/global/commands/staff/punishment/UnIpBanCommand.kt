...
class UnIpBanCommand : Command(
    ...
    permission = Permission("minepact.punishments.unban-ip"),
    cooldown = 1.0,
) {
    override fun execute(
        ...
        PunishmentRepository.findActiveByTargetAndType(PlayerRegistry.get(targetName).get().data.uuid, PunishmentType.IP_BAN).thenAccept { ban ->
            if (ban == null) {
                sender.sendMessage("<red>No active ip-ban found for <white>$targetName")
                return@thenAccept
            }
