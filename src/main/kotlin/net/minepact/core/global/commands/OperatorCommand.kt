...
class OperatorCommand : Command(
    ...
    permission = Permission("minepact.admin.op"),
    ...
    override fun execute(
        ...
        try {
            ...
            if (player.isOp) {
                sender.sendMessage("<red>${player.name} is already an operator!")
                return Result.FAILURE
            }
            player.isOp = true
            sender.sendMessage("<green>${player.name} is now an operator!")
